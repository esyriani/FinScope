...
Settings stores runtime preferences in the active database. General settings are personal to each signed-in user. Owner-only settings control shared behavior for the deployment and are saved on the owner account so request, background, and script workflows resolve the same values.
