...
The diagram shows the stable runtime boundaries reflected by `finance_app.__init__.create_app`, the registered blueprints in `finance_app.modules`, and the repository-level `runtime/` data directory.

![Application architecture diagram](diagrams/architecture-diagram.png)

The editable Mermaid source is stored in `docs/diagrams/architecture.mmd`. Keep the rendered SVG generated from that source when the architecture changes.

## Module structure

Feature code lives under `src/finance_app/modules`. A module should keep presentation, logic, and persistence concerns separate where practical.

Common files:

```text
controller.py     Flask routes and HTTP request/response handling.
service.py        Use-case orchestration and page context builders.
forms.py          Form parsing and validation.
presenter.py      View-model shaping for templates.
queries.py        Read-side SQL.
repository.py     Mutation persistence helpers.
workflow.py       Multi-step application workflows.
engine.py         Domain logic that is independent from Flask.
```
...
Translation catalogs are JSON files under `src/finance_app/translations/`. Keep user data, merchant names, category names, account names, and uploaded statement content untranslated.

## Current module examples

```text
src/finance_app/modules/transactions/
  controller.py
  service.py
  filters.py
  queries.py
...
Keep those in `runtime/` or another protected local path.
