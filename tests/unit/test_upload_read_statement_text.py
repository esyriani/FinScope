"""Tests for statement text extraction in the upload service."""
...
def test_read_statement_text_rejects_pdf_files():
    ...
    assert upload_service.read_statement_text(uploaded_file, "pdf") is None
...
def test_read_statement_text_rejects_unsupported_file_types():
    ...
    assert upload_service.read_statement_text(uploaded_file, "txt") is None
...
def test_read_statement_text_decodes_csv_with_utf8_sig():
    ...
    assert upload_service.read_statement_text(uploaded_file, "csv") == "Date,Description,Amount\n"
