...
def test_transaction_merchant_filter_builds_sql_candidate_predicate(db_conn):
    ...
    with captured_sql() as statements:
        core_filters = build_transaction_core_filters(filters, "UNKNOWN", conn=db_conn)
...
def test_rule_candidate_filter_builds_sql_amount_and_description_predicates(db_conn):
    ...
    condition = rule_sql_candidate_condition(db_conn, rule, "METRO")
...
def test_review_merchant_filter_executes_sql_candidate_predicate(db_conn):
    """Verify review merchant searches are pushed into the SQL query."""
    with captured_sql() as statements:
        review_candidate_rows(db_conn, "UNKNOWN", merchant_candidate="hydro")
...
def test_dashboard_category_aggregation_uses_sql_grouping(db_conn):
    """Verify category analytics are aggregated with SQL grouping."""
    with captured_sql() as statements:
        fetch_spending_by_category(db_conn, [transactions_table.c.ignored == 0], "UNKNOWN")
...
def test_comparison_period_summary_aggregates_with_sql_date_filters(db_conn):
...
def test_calendar_month_transactions_apply_sql_filters_before_presenting_rows(db_conn):
