...
def insert_rule_engine_transaction(
    conn,
    description,
    amount,
    fingerprint,
...
def test_apply_single_rule_to_transactions_updates_matching_active_rows(core_conn):
    ...
    matching_id = insert_rule_engine_transaction(core_conn, "Metro Grocery #123", 12.34, "single-match")
    out_of_range_id = insert_rule_engine_transaction(core_conn, "Metro Grocery #456", 30.00, "single-range")
    ignored_id = insert_rule_engine_transaction(core_conn, "Metro Grocery #789", 12.34, "single-ignored", ignored=1)
    rule_id = insert_rule(core_conn, "METRO", "Food", amount_min=10, amount_max=20, tags=["Tax"])
    ...
    matching = transaction_state(core_conn, matching_id)
    out_of_range = transaction_state(core_conn, out_of_range_id)
    ...
    assert get_transaction_tag_names(core_conn, matching_id) == ["Tax"]
...
def test_rules_respect_account_and_direction_constraints(core_conn):
    ...
    savings_id_tx = insert_rule_engine_transaction(
        core_conn,
        "Metro Grocery",
        12.34,
    ...
    credit_id = insert_rule_engine_transaction(
        core_conn,
    ...
    updated_count = apply_all_rules_to_transactions(core_conn)
    core_conn.commit()

    checking = transaction_state(core_conn, checking_match_id)
    savings = transaction_state(core_conn, savings_id_tx)
    credit = transaction_state(core_conn, credit_id)
    assert updated_count == 1
...
def test_merchant_bound_rule_matches_only_the_bound_merchant(core_conn):
    """Verify merchant-bound rules use merchant IDs instead of fuzzy keywords."""
    metro_id = get_or_create_merchant_for_description(core_conn, "Metro Grocery #123")["id"]
    other_id = get_or_create_merchant(
        core_conn,
        "OTHER MERCHANT",
    )["id"]
    matching_id = insert_rule_engine_transaction(
        core_conn,
        "Metro Grocery #123",
        12.34,
    ...
    other_merchant_id = insert_rule_engine_transaction(
        core_conn,
        "Metro Grocery #456",
        12.34,
    ...
    rule_id = insert_merchant_rule(core_conn, metro_id, "METRO GROCERY", "Food", tags=["Tax"])
    rule = {
        "id": rule_id,
        "merchant_id": metro_id,
    ...
    preview_count, preview_sample = preview_rule_matches(core_conn, rule, limit=5)
    updated_count = apply_single_rule_to_transactions(core_conn, rule)
    core_conn.commit()

    assert preview_count == 1
    assert [row["id"] for row in preview_sample] == [matching_id]
    assert updated_count == 1
    assert transaction_state(core_conn, matching_id)["category_rule_id"] == rule_id
    assert transaction_state(core_conn, other_merchant_id)["category"] == "UNKNOWN"


def test_core_preview_and_single_rule_application(app, core_conn):
    """Verify preview and single-rule application can run through Core."""
    del app
    matching_id = insert_rule_engine_transaction(core_conn, "Metro Grocery #123", 12.34, "core-single-match")
    out_of_range_id = insert_rule_engine_transaction(core_conn, "Metro Grocery #456", 30.00, "core-single-range")
    insert_rule_engine_transaction(core_conn, "Metro Grocery #789", 12.34, "core-single-ignored", ignored=1)
    rule_id = insert_rule(core_conn, "METRO", "Food", amount_min=10, amount_max=20, tags=["Tax"])
    rule = {
        "id": rule_id,
        "merchant_id": None,
    ...
    assert updated_count == 1
    assert transaction_state(core_conn, matching_id)["category_rule_id"] == rule_id
    assert transaction_state(core_conn, out_of_range_id)["category"] == "UNKNOWN"
    assert get_transaction_tag_names(core_conn, matching_id) == ["Tax"]


def test_apply_all_rules_to_transactions_captures_undo_and_skips_noops(core_conn):
    """Verify all-rule application updates changed rows and captures undo snapshots."""
    metro_id = insert_rule_engine_transaction(core_conn, "Metro Grocery", 20.00, "all-metro")
    payroll_id = insert_rule_engine_transaction(core_conn, "Payroll Deposit", -1000.00, "all-payroll")
    already_id = insert_rule_engine_transaction(
        core_conn,
        "Cafe Bistro",
        15.00,
        "all-already",
    ...
    metro_rule_id = insert_rule(core_conn, "METRO", "Food", tags=["Tax"])
    ...
    updated_count, undo_changes = apply_all_rules_to_transactions(core_conn, capture_undo=True)
    core_conn.commit()

    assert updated_count == 2
    assert {change["transaction_id"] for change in undo_changes} == {metro_id, payroll_id}
    assert transaction_state(core_conn, metro_id)["category_rule_id"] == metro_rule_id
    assert transaction_state(core_conn, payroll_id)["category_rule_id"] == payroll_rule_id
    assert transaction_state(core_conn, already_id)["category"] == "Food"
    assert get_transaction_tag_names(core_conn, metro_id) == ["Tax"]


def test_core_apply_all_rules_uses_deterministic_merchant_keys(app, core_conn):
    """Verify Core all-rule application matches deterministic merchant keys."""
    del app
    amazon_id = insert_rule_engine_transaction(core_conn, "AMZN Mktp CA*ZZ999", 20.00, "core-alias-amazon")
    rule_id = insert_rule(core_conn, "AMZN MKTP", "Food", tags=["Tax"])
    ...
    assert transaction_state(core_conn, amazon_id)["category_rule_id"] == rule_id
    assert get_transaction_tag_names(core_conn, amazon_id) == ["Tax"]


def test_apply_all_rules_preserves_square_processor_merchant_token(app, core_conn):
    """Verify Square payment descriptors keep the starred merchant for rule matching."""
    del app
    tx_id = insert_rule_engine_transaction(core_conn, "SQ *COSMETA", 74.73, "square-cosmeta")
    rule_id = insert_rule(core_conn, "COSMETA", "Food", tags=["Tax"])
    ...
    assert transaction_state(core_conn, tx_id)["category_rule_id"] == rule_id
    assert get_transaction_tag_names(core_conn, tx_id) == ["Tax"]


def test_apply_all_rules_job_and_undo_restore_previous_values(app, core_conn):
    """Verify the background all-rules job can restore transaction state."""
    metro_id = insert_rule_engine_transaction(core_conn, "Metro Grocery", 20.00, "job-metro")
    insert_rule(core_conn, "METRO", "Food", tags=["Tax"])
    undo_state = {}

    message = apply_all_rules_job(undo_state)

    applied = transaction_state(core_conn, metro_id)
    ...
    restored = transaction_state(core_conn, metro_id)
    ...
    assert get_transaction_tag_names(core_conn, metro_id) == []


def test_apply_all_rules_job_rolls_back_when_late_tag_write_fails(app, core_conn, monkeypatch):
    """Verify all-rule jobs do not persist partial category updates on failure."""
    metro_id = insert_rule_engine_transaction(core_conn, "Metro Grocery", 20.00, "job-rollback")
    insert_rule(core_conn, "METRO", "Food", tags=["Tax"])
    ...
    state = transaction_state(core_conn, metro_id)
...
def test_undo_apply_all_rules_job_skips_transactions_changed_after_job(app, core_conn):
    """Verify undo does not overwrite transactions edited after rule application."""
    metro_id = insert_rule_engine_transaction(core_conn, "Metro Grocery", 20.00, "job-skip")
    ...
    core_conn.execute(
        ...
        .values(category="Personal", category_id=resolve_category_id(core_conn, "Personal"))
    )
    core_conn.commit()
